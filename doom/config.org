...
(add-hook 'doom-load-theme-hook
  (defun my/darken-solaire-bg ()
    (when (facep 'solaire-default-face)
      (set-face-attribute 'solaire-default-face nil :background "#2A2B32"))))
...
user has scrolled up to read history. When the user scrolls up, we enter
=vterm-copy-mode= which freezes the display (the underlying process keeps
running). Scrolling back to the bottom exits copy mode automatically.
...
*** Open vterm in a bottom split at the project root
...
*** Open vterm in a bottom split at the current file's directory
